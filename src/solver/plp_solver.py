...
class SudokuSolver():
    ...
    def __init__(self) -> None:
        ...
        To solve multiple sudoku problems, class can be initialised only once
        """
    ...
    def _solution_to_numpy(self, solution) -> np.ndarray:
        ...
        Args:
            solution (Any)): solution generated by plp.LpProblem.solve() 

        Returns:
            np.ndarray: matrix containing solved sudoku
        """

        solution = [[0 for _ in self._cols] for _ in self._rows]
        for row in self._rows:
            for col in self._cols:
                for value in self._values:
